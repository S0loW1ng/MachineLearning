...
# Performing PCA onto training dataframe, from 4096 features to 2.
...
plt.show()
"""
# Recovering 64x64 image after going through PCA and reducing 4096 dimensions to 2 (Just for comparison purposes)
...
model = MLPClassifier(hidden_layer_sizes=(20, 10, 5), activation='relu', solver='adam', max_iter=100000,
...
plt.contourf(xx, yy, Z, alpha=0.4, cmap=plt.cm.get_cmap('Spectral', 10))
plt.scatter(X_pca_train[:, 0], X_pca_train[:, 1], c=y_train, alpha=0.8, cmap=plt.cm.get_cmap('Spectral', 10))
plt.xlim(-30, 50)
plt.ylim(-10, 10)
plt.title('MLP Classifier Boundary with Training Set')
...
plt.figure()
plt.contourf(xx, yy, Z, alpha=0.4, cmap=plt.cm.get_cmap('Spectral', 10))
plt.scatter(X_pca_test_centered[:, 0], X_pca_test_centered[:, 1], c=y_test, alpha=0.8,
            cmap=plt.cm.get_cmap('Spectral', 10))
plt.xlim(-30, 50)
plt.ylim(-10, 10)
plt.title('MLP Classifier Boundary with Testing Set')
plt.xlabel("Principal Component 1")
plt.ylabel("Principal Component 2")
plt.show()
